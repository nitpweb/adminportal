...
const handler = async (req, res) => {
    ...
    `
        ...
        SELECT * from innovation where openDate<? and closeDate>? ORDER BY openDate DESC`,
            ...
            results = await query(
                `
			SELECT * from innovation where closeDate<=? and openDate>=? ORDER BY openDate DESC limit ?,?`,
            ...
            results = await query(
                `SELECT * from innovation ORDER BY openDate DESC limit ?, ?`,
                [from, to - from]
            )
        } else {
            results = await query(`SELECT * from innovation WHERE id=?`, [
                String(type),
            ])
        }
